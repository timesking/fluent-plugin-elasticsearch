...
class ElasticsearchOutput < Test::Unit::TestCase
  ...
  def test_writes_to_logstash_timezone_set
    driver.configure("logstash_format true
                      timezone Asia/Shanghai")
    # Nomatter `utc_index false` or `utc_index true`, the index time will be generated by timezone
    # ingestion time
    dt = DateTime.new(2015, 5, 31, 16, 0, 1, "+00:00")
    logstash_index = "logstash-2015.06.01"
    stub_elastic_ping
    stub_elastic
    driver.emit(sample_record, dt.to_time.to_i)
    driver.run
